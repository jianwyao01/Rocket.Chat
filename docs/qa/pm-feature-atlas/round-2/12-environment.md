...
| 稳定语义 id | 族 | API | 功能一句话 | 触发入口 | 效果 | 关联 | 出处 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `env.clipboard.useClipboardWithToast` | clipboard | `useClipboard(` | 包装 fuselage `useClipboard`，成功/失败 toast | 被下面 8 个 `useClipboardWithToast(` 调用 | Plan/Webhook/Invite 点 Copy 出 Copied toast（shot p6b-plan-site.png） | 各 `env.clipboard.*Toast` 行 | `apps/meteor/client/hooks/useClipboardWithToast.ts:10` [实测] |
...
| `env.clipboard.IncomingWebhookForm.url` | clipboard | `useClipboardWithToast(` | 复制 Incoming webhook URL | Admin→Integrations→Incoming→Webhook URL | Copied toast + 剪贴板 /hooks/…（shot p6b-hook-url.png） | 同表 token/curl | `apps/meteor/client/views/admin/integrations/incoming/IncomingWebhookForm.tsx:60` [实测] |
| `env.clipboard.IncomingWebhookForm.token` | clipboard | `useClipboardWithToast(` | 复制 Incoming token | 同上 Token | Copied toast + 剪贴板 id/token（shot p6b-hook-token.png） | 同表 url/curl | `apps/meteor/client/views/admin/integrations/incoming/IncomingWebhookForm.tsx:61` [实测] |
| `env.clipboard.IncomingWebhookForm.curl` | clipboard | `useClipboardWithToast(` | 复制 curl 示例 | 同上 curl | Copied toast + 剪贴板 curl -X POST（shot p6b-hook-curl.png） | 同表 url/token | `apps/meteor/client/views/admin/integrations/incoming/IncomingWebhookForm.tsx:62` [实测] |
| `env.clipboard.PlanCard.siteURL` | clipboard | `useClipboardWithToast(` | 复制订阅页 Site URL | Admin→Subscription→Plan 卡 | Copied toast + 剪贴板 http://127.0.0.1:3000（shot p6b-plan-site.png） | 同表 hashed | `apps/meteor/client/views/admin/subscription/components/cards/PlanCard/PlanCardLicenseDetails.tsx:20` [实测] |
| `env.clipboard.PlanCard.hashed` | clipboard | `useClipboardWithToast(` | 复制 hashed Site URL | 同上 | Copied toast + 剪贴板 hashed hex（shot p6b-plan-hash.png） | 同表 siteURL | `apps/meteor/client/views/admin/subscription/components/cards/PlanCard/PlanCardLicenseDetails.tsx:21` [实测] |
| `env.clipboard.InviteLink` | clipboard | `useClipboardWithToast(` | 复制房间邀请链接 | 房间成员栏→Invite_Link | Copied toast + go.rocket.chat/invite?host=…（shot p6b-invite-copy.png） | `env.deeplink.invite.route` | `apps/meteor/client/views/room/contextualBar/RoomMembers/InviteUsers/InviteLink.tsx:15` [实测] |
...
| `env.download.useDownloadExportMutation` | download | `downloadJsonAs(` | 导出选中消息为 JSON 文件 | 房间工具 Export_Messages→Download file | Export 选 19 条后 Download → exportedMessages-*.json（shot p5-export-json.png） | `perm.mail-messages` | `apps/meteor/client/views/room/contextualBar/ExportMessages/useDownloadExportMutation.ts:60` [实测] |
| `env.download.useExportMessagesAsPDFMutation` | download | `link.download =` | 导出选中消息为 PDF | Export_Messages→PDF（`export-messages-as-pdf`） | toast Messages exported successfully + exportedMessages-*.pdf（shot p5-export-pdf.png） | `perm.export-messages-as-pdf` | `apps/meteor/client/views/room/contextualBar/ExportMessages/useExportMessagesAsPDFMutation.tsx:171` [实测] |
...
| `env.download.FileItemMenu.download` | download | `download(` | 房间文件栏：明文文件直接下载 | 同上菜单（非 decrypt 路径） | Files→More→Download atlas-download.txt（shot p5-files-dl.png） | `env.download.anchor.download` | `apps/meteor/client/views/room/contextualBar/RoomFiles/components/FileItemMenu.tsx:71` [实测] |
...
| `env.deeplink.useLoginOtherClients` | deep link | `buildDeepLinkURL(` | 已有 resumeToken 时交给桌面/移动 | URL `?resumeToken=&userId=&loginClient=` | resumeToken+userId+loginClient=desktop 弹出 Open xdg-open?（shot p6-login-other.png） | 同上 | `apps/meteor/client/views/root/hooks/useLoginOtherClients.ts:21` [实测] |
...
| `env.deeplink.ThreadMessageList.clear` | deep link | `setMessageJumpQueryStringParameter(` | 线程内 jump 完成后清 `?msg=` | 上一项完成 | 打开 /thread/…?msg= 后 URL 去掉 msg=，回复仍黄底（shot p6b-thread-clear.png） | 同上 | `apps/meteor/client/views/room/contextualBar/Threads/components/ThreadMessageList.tsx:340` [实测] |
...
| `env.storage.useDraft.removeItem` | storage | `localStorage.removeItem` | 草稿空或已 flush 后删本地键 | 清空/flush | composer 清空后 messagebox_* 键消失（shot p6b-draft-remove.png） | 同上 | `apps/meteor/client/views/room/composer/messageBox/hooks/useDraft.ts:13` [实测] |
