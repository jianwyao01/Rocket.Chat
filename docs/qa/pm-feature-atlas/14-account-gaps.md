...
本表 NEW **12**。计数：`rg -c '^\| `account\.gap\.' docs/qa/pm-feature-atlas/14-account-gaps.md`。
...
### 4.7 B7 Tokens（9）— **对照完整**

`name` `name.required` `bypass-2fa` `add` `regenerate` `remove` `pagination` `empty` `retry`

create / show-once（捆在 add+regenerate 后果）/ regenerate / delete 齐。无 NEW。
...
| `account.tokens` | `/account/tokens` `:72-74` | `:39-43` | B7 9 | 完整 |
...
| --- | --- |
| profile 校验 | 3 |
| security 备份码 Copy + E2E enter/forgot | 7 |
| sessions 空态/重试 | 2 |
| **合计** | **12** |
...
去重：`rg -o '^\| `account\.gap\.[^`]+' docs/qa/pm-feature-atlas/14-account-gaps.md | sort -u | wc -l` → 12。
...
- **B3 Security** + `totp.backup-copy` + 5 条 enter + `e2e-forgot` `e2e-forgot-confirm`
...
`[读]` 源码推断。`[待渲染实测]` 未挂真实 UI。不改 07/04 原文。不把 composer WebDAV Add 或房间 `SaveE2EPasswordModal` Copy 写成账号页控件。不把 03 的 9 条 shortcut 绑定再写一遍。
