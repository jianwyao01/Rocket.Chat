...
Committed - pending implementation
...
- **Why:** apps-engine's 16 `IPre*` handlers use five inconsistent return
  contracts today (boolean, entity-object, `IEmailDescriptor`, void+throw), and
  none of them can express *"prompt the user and proceed only if they
  accept."* `prompt` is the one genuinely new capability this proposal adds;
  `pass`/`patch`/`prevent` just unify mechanisms that already exist across
  messages, rooms, uploads, and email.
...
- **How it ships:** bootstrap on **new media call event hooks** for apps
  (Strategy A — `EventResult`'s first real usage, implemented later in this
  branch), then **widen existing handlers** to their restricted `EventResult`
  union per event (Strategy B, the destination) — semantics unchanged.
- **Scope right now:** this document stands on its own — it defines the type,
  the guard, the authoring API, and the rollout for *all* apps-engine
  pre-events. It is written to serve as the shared base that future
  event-result work builds on, rather than a one-off attached to any single
  event.
...
Apps-engine's pre-event handlers return five inconsistent contracts today
(boolean, entity-object, `IEmailDescriptor`, void+throw — see
[Current state](#current-state-what-pre-events-return-today)). None of them
can express *"prompt the user and proceed only if they accept,"* and there is
no single vocabulary an app author can reach for regardless of which event
they are hooking. This document proposes a single, uniform `EventResult`
return type for **all** apps-engine pre-events — `pass` / `patch` / `prevent` /
`prompt` — meant as the base that future pre-event (and eventually post-event)
return contracts build on, rather than growing a sixth one-off shape the next
time a new capability is needed.

`EventResult`'s first real usage will be the new media call event hooks for
apps, implemented later in this branch (see
[Rollout strategy](#rollout-strategy-bootstrap-with-a-converge-on-b)). The
`prompt` variant in particular is worked out in detail below because it is
the capability the
[file-upload confirmation proposal](./app-file-upload-confirmation.md) needs, but it lives elsewhere
— *"prompt the user and proceed only if they accept"* — which no pre-event
can express today. `pass`, `patch`, and `prevent` unify mechanisms already
used across messages, rooms, uploads, and email, independent of that
proposal, and the type is meant to be adopted event-by-event over time.
...
[file-upload confirmation proposal](./app-file-upload-confirmation.md)
describes.
...
9. **Upload confirmation is the first application of `EventResult.prompt`**,
   via a new upload handler (see the
   [file-upload confirmation proposal](./app-file-upload-confirmation.md)),
   superseding the standalone `IUploadConfirmationRequest`.
...
**before** any legacy branch. The `@` prefix keeps it out of the space of
legitimate property names an entity or a future field might plausibly use, so
there is zero overlap with `IMessage`/`IRoom`/`IEmailDescriptor`. Authors never
hand-write the marker; the `EventResult.*` factories stamp it (§"Authoring API").
Internally the manager uses a `MarkedEventResult` type carrying `@kind`; the
public `EventResult<T>` type authors annotate against omits it.
...
```ts
// e.g. for uploads (the first event to adopt EventResult)
...
- **Incremental** — ship `EventResult` for the media call event hooks first
  (implemented later in this branch), then add message/room/email variants.
...
- **File upload — yes.** The two-step `rooms.media` → `rooms.mediaConfirm` flow
  ...
  step — this is the [file-upload confirmation proposal](./app-file-upload-confirmation.md).
  These media call event hooks are `EventResult`'s first real usage, to be
  implemented later in this branch. Note that upload's `prevent` and `prompt`
  live on **different handlers**: hard blocks stay on the block-only
  `IPreFileUpload` at the `rooms.media` pre-stage, while `prompt`/`pass` are on
  the new confirmation handler at `rooms.mediaConfirm`.
...
2. Implement Strategy A for the **media call event hooks** first (to be
   implemented later in this branch), returning the correct `EventResult` variants
   This establishes the pattern later events (message, room, email) reuse.
