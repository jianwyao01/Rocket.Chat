 ...
 */
export type IUnansweredMediaCallEndedContext = IMediaCallEndedContext & {
	call: Omit<IMediaCall, 'acceptedAt'> & { acceptedAt?: undefined };
};

/** An ended call the callee joined. `acceptedAt` is the moment they accepted. */
export type IAnsweredMediaCallEndedContext = IMediaCallEndedContext & {
	call: Omit<IMediaCall, 'acceptedAt'> & { acceptedAt: Date };
};

/** An ended call the callee actively declined. */
export type IRejectedMediaCallEndedContext = IUnansweredMediaCallEndedContext & { hangupReason: 'rejected' };
...
export function isRejectedCall(context: IMediaCallEndedContext): context is IRejectedMediaCallEndedContext {
	return !isAnsweredCall(context) && context.hangupReason === 'rejected';
}
 ...
 *
 * Do not test `hangupReason === 'not-answered'` instead. That value is written by
 * the *caller's* client when its ring timeout fires. A caller that closes the tab
