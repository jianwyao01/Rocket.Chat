 ...
 *
 * This list is a **copy**. The source of truth is `callHangupReasonList` in
 * `@rocket.chat/media-signaling`, plus the codes the server passes to
 * `hangupByServer`. The Apps-Engine publishes the app-facing SDK on its own and
 * must not depend on an internal package, so the values are duplicated here.
 * `appEvents.spec.ts` asserts that this copy still covers the original.
 */
export const mediaCallHangupReasonList = [
	...
	'another-client',
	/** A SIP REFER failed while the call was transferred. */
	'sip-refer-failed',
] as const;

/**
 * A reason Rocket.Chat is known to record. SIP failures carry the response code
 * they came from, e.g. `'sip-error-486'`.
 */
